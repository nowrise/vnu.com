...
const NowRiseApplications = () => {
  const queryClient = useQueryClient();

  const { data: applications, isLoading } = useQuery({
    queryKey: ["admin-nowrise-applications"],
    queryFn: async () => {
  ...
  const updateStatus = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-nowrise-applications"] });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">NowRise Institute Applications</h1>
      ...
      {applications && applications.length > 0 ? (
        ...
        <div className="text-center py-12 bg-secondary/30 rounded-lg">
          ...
          <h3 className="text-lg font-medium mb-2">No NowRise applications yet</h3>
          <p className="text-muted-foreground">
            Applications will appear here when students apply for NowRise programs.
          </p>
        </div>
      )}
    </div>
...
export default NowRiseApplications;
